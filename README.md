...
| 我的余额 | 所有用户 | 查看余额、单价与打印扣费 / 退费记录；**管理员/anticraft/白名单免费**，其余账号按张计费（充值/付款码暂未实现） |
| 任务信息页 | — | 每单出纸前自动打一张封面页：提交人 / 文件 / 配送地址 / 提交时间 / 打印时间；管理设置里可关 |
| 站点图标 | — | `frontend/public/favicon.{svg,ico}` + `apple-touch-icon.png`；改图标跑 `frontend/scripts/make_favicon.py` |
