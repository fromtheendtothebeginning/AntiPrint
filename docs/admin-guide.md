...
| `admin` 管理员 | 上面全部 + 任务队列（审核/驳回/交接勾选）、预览文件、管理设置（二级菜单：打印设置/计费/两个名单/anticraft/代理）、用户列表（只读） |
...
- **白名单**：「管理设置 → 免费白名单」分栏是一张表格，输入用户名点「添加」即免打印费，行尾「移除」即取消；表格会标出这个名字在本站有没有对应账号（写错的名字不会生效，会显示「本站没有这个账号」）。
- **余额**：「用户管理 → 调整余额」（管理员可操作）：正数加钱、负数扣钱，备注会记进该账号的消费记录。**充值与付款码暂未实现**，
