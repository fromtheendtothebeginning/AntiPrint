...
|---|---|
| 份数 | 1~99 |
| 单/双面 | 单面、双面（长边翻转）、双面（短边翻转） |
| 纸张大小 | A4 / A3 / A5 / B5 / Letter / Legal |
| 颜色 | 黑白、彩色（目标机是黑白激光时彩色项会被驱动忽略） |
| 页面范围 | 形如 `1-3,5`，留空 = 全部 |
| 每张纸页数 | 1 页 / 2 页（左右） / 2 页（上下） / 4 页 / 9 页 / 16 页 |
| 缩放 | 适应纸张 / 实际大小 / 缩小到可打印区域 |

这些设置随任务下发给打印代理，代理用 SumatraPDF 的 `-print-settings` 应用；「我的任务」与「任务队列」都会显示一行摘要（如「2 份 · 双面（长边翻转） · A3 · 黑白 · 第 1-2 页 · 每张 4 页 · 适应纸张」）。
注意：目标机型 `HP LaserJet Professional P1106` **没有自动双面单元、只支持黑白**，双面/彩色设置会照常下发，实际效果由打印机驱动决定。
