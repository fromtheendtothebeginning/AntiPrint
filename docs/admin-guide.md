...
用户在「提交打印」页可以直接选（默认单面 / A4 / 黑白 / 1 页每张 / 适应纸张 / 1 份）：
...
这些设置随任务下发给打印代理，代理用 SumatraPDF 的 `-print-settings` 应用；「我的任务」与「任务队列」都会显示一行摘要（如「2 份 · A3 · 第 1-2 页 · 每张 4 页 · 适应纸张」）。

**预览**：用户在提交页**把文件拖进（或选中）后，投放区就变成预览面板**——PDF 直接内嵌显示、图片显示缩略图，面板上有「放大查看（弹窗）/ 继续添加 / 清空」，点下方文件名可切换预览哪一个；也可以继续往面板里拖文件追加。提交后到「我的任务」点文件名同样能预览自己上传的件；管理员在「任务队列」点文件名预览待审文件。
