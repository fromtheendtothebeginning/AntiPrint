...
| 管理员 | `end`（口令由部署者设定） |
| 默认账号 | 播种的 `admin`（id=21）**口令仍是默认的 `admin123`**（2026-09-21 实测可登录成功，与旧记录「已随机化、401」不符）——**请用它以外的账号管理本站**（`end` / `miemieYLF` / `19337621235`），并尽快改掉或停用它 |
...
| `admin` 管理员 | 上面全部 + 任务队列（审核/驳回/交接勾选）、预览文件、管理设置（二级菜单）、用户管理（免费账户开关、调整余额；加管理员与删除账号仅 root） |
...
- **我的配置**：用户可以设置默认配送地址/方式、绑定 anticraft 账号，以及**上传头像**（png/jpg/gif/webp，≤2MB；显示在侧栏）。
- **任务队列**：审核（同意 / 驳回，驳回必填理由）、预览上传的文件、打印完成后勾选交接状态；每行的操作按钮**始终显示**（手机上不用悬停）；**手机端**「文件与设置」收成一个带文件数的图标，点开看文件名、大小与打印设置，点文件名即可预览；**手机端点任务行会从屏幕底部弹出详情抽屉**（提交时间、地址、备注、文件与设置都在里面），**同意 / 驳回 / 重新打印 / 交接勾选 / 删除 都在抽屉底部的大按钮上**（贴着屏幕下沿，单手就能点到）；每行还有 **`重新打印`**（已出纸/已结束的任务再打一份，会把任务打回「已通过」重新入队）与 **`删除`**（二次确认，连上传文件一起删，不可恢复）。
...
## 8.5 打印计费与账户余额（2026-09-15 新增）

- **谁要花钱**：普通本地账号按「张数 × 单价」扣余额；**管理员/root、anticraft 账号、白名单**免费。
- **任务信息页（封面页）**：默认**每单出纸前先打一张**，写清任务号 / 提交人 / 文件名 / 份数 / 配送方式与地址 / 提交时间 / **打印时间**，线下交付时不用对着屏幕找。不想多耗纸可在「管理设置 → 打印设置」里取消勾选「打印任务信息页」（`cover_page`）。
- **管理设置页已按二级菜单分栏**：打印设置 / 打印计费 / 免费白名单 / 管理员名单 / anticraft 绑定 / 打印代理，点顶部按钮切换；名单类表格增删**即时生效**，单价等表单要**点「保存设置」**才生效。
- **单价**：「管理设置 → 打印计费 → 单价（元/张）」，默认 **0.1 元**，可改（0 ~ 100，两位小数）。**改完点保存**才生效。
- **白名单**（免打印费名单）：「用户管理」页每行有「设为免费 / 取消免费」按钮（即时生效，行里会出现「免费」徽章）；页顶还有「**批量添加免费账号**」——一行一个（或逗号分隔）粘一批用户名即可，**可以直接写还没注册的用户名**：他们用这个名字注册/登录后会自动成为免费账号；下方会把「还没注册的名字」单独列出来，可随时移除。表格会标出这个名字在本站有没有对应账号（写错的名字不会生效，会显示「本站没有这个账号」）。
- **余额**：「用户管理 → 调整余额」（管理员可操作）：正数加钱、负数扣钱，备注会记进该账号的消费记录。
- **删除账号**：「用户管理 → 删除」（管理员与超级管理员都可以）：**余额必须为 0** 才能删；只删账号，他提交过的任务与余额流水都保留（任务列表里提交人显示为空）。**充值与付款码暂未实现**，
  所以现在的流程是：线下收款 → 管理员手工加余额。
- **扣费规则**：提交任务时按「页数 ÷ 每张页数 × 份数」扣；页面范围、每张页数（nup）、份数都会算进去；Word/PPT 按转换后的 PDF 页数算。
  余额不足时**不会建单**，界面弹出「付款码（暂未开放）」占位页。
- **退费**：被**驳回**、用户**撤回**、或管理员删除**尚未出纸**的任务，钱会自动退回余额；**打印失败不退**（可以「重新入队 / 重新打印」，不会重复扣）。
- **用户看得到**：侧栏「我的余额」页显示余额、账号类型、单价与最近的扣费/退费/调账记录。
- **排查**：提交报「余额不足」但余额够 → 检查是不是单价被改大了、或者这个账号被移出了白名单；「我的余额」页的消费记录能看到每一笔。
